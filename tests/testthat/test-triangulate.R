...
test_that("earcut splits a square into two triangles", {
  ...
  sfc <- sf::st_as_sfc(geoarrow::as_geoarrow_vctr(ga_triangulate_earcut(g)))
...
test_that("delaunay preserves length", {
  ...
  sfc <- sf::st_as_sfc(geoarrow::as_geoarrow_vctr(ga_triangulate_delaunay(g)))
  expect_length(sfc, 2L)
})

test_that("delaunay defaults to constrained", {
  g <- ga(sf::st_sfc(square()))
  a <- sf::st_as_sfc(geoarrow::as_geoarrow_vctr(ga_triangulate_delaunay(g)))
  b <- sf::st_as_sfc(geoarrow::as_geoarrow_vctr(ga_triangulate_delaunay(
    g,
    TRUE
  )))
...
test_that("unconstrained triangulation ignores holes, constrained does not", {
  ...
  con <- sf::st_as_sfc(geoarrow::as_geoarrow_vctr(ga_triangulate_delaunay(
    g,
    TRUE
  )))
  unc <- sf::st_as_sfc(geoarrow::as_geoarrow_vctr(ga_triangulate_delaunay(
    g,
    FALSE
  )))
...
test_that("snap_radius recycles and rejects a wrong length", {
  ...
  expect_no_error(ga_triangulate_delaunay(g, TRUE, 1e-4))
  expect_no_error(ga_triangulate_delaunay(g, TRUE, c(1e-4, 1e-3)))
  expect_error(
    ga_triangulate_delaunay(g, TRUE, c(1e-4, 1e-3, 1e-2)),
