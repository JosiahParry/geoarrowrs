...
test_that("contains and within are mirror images", {
  ...
  expect_true(conv(contains(x, y)))
  expect_false(conv(within(x, y)))
  expect_true(conv(within(y, x)))
})
...
test_that("contains matches sf", {
  ...
  expect_equal(
    conv(contains(ga(outer), ga(inner))),
...
test_that("intersects and disjoint are negations", {
  ...
  expect_equal(conv(intersects(x, y)), c(TRUE, FALSE))
  expect_equal(conv(disjoint(x, y)), c(FALSE, TRUE))
})
...
test_that("touches is TRUE for shared edges only", {
  ...
  expect_equal(conv(touches(x, y)), c(TRUE, FALSE))
})

test_that("overlaps needs partial intersection of equal dimension", {
  x <- ga(sf::st_sfc(box(0, 0, 2, 2), big()))
  y <- ga(sf::st_sfc(box(1, 1, 3, 3), small()))

  expect_equal(conv(overlaps(x, y)), c(TRUE, FALSE))
})
...
test_that("covers is weaker than contains on a boundary point", {
  ...
  expect_true(conv(covers(x, edge)))
  expect_false(conv(contains(x, edge)))
})

test_that("covered_by mirrors covers", {
  x <- ga(sf::st_sfc(big()))
  edge <- ga(sf::st_sfc(pt(0, 2)))

  expect_true(conv(covered_by(edge, x)))
})

test_that("contains_properly is stricter than contains", {
  x <- ga(sf::st_sfc(big(), big()))
  y <- ga(sf::st_sfc(small(), pt(0, 2)))

  expect_equal(conv(contains_properly(x, y)), c(TRUE, FALSE))
})
...
test_that("equals_topo ignores winding and start point", {
  ...
  expect_true(conv(equals_topo(x, y)))
})
...
test_that("predicates recycle y", {
  ...
  expect_equal(conv(contains(x, y)), rep(TRUE, 3))
})
...
test_that("relate agrees with the named predicates", {
  ...
  expect_equal(conv(relate(x, y)), "212FF1FF2")
  expect_true(conv(contains(x, y)))
})
...
test_that("is_empty distinguishes empty from populated", {
  g <- ga(sf::st_sfc(big(), sf::st_polygon()))
  expect_equal(conv(is_empty(g)), c(FALSE, TRUE))
})
...
test_that("coordinate_position recycles a single point", {
  ...
  expect_equal(conv(coordinate_position(g, p)), c("inside", "inside"))
})
...
test_that("topology reads a concrete array from a reader", {
  ...
  expect_length(conv(intersects(g, p)), 100L)
  expect_length(conv(dimension(g)), 100L)
  expect_true(any(conv(coordinate_position(g, p)) == "outside"))
})
