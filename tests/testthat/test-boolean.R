...
test_that("boolean_intersection keeps the shared area", {
  ...
  expect_equal(area(ga_boolean_intersection(x, y)), 1)
})
...
test_that("boolean_union keeps the combined area", {
  ...
  expect_equal(area(ga_boolean_union(x, y)), 7)
})
...
test_that("boolean_difference removes the overlap", {
  ...
  expect_equal(area(ga_boolean_difference(x, y)), 3)
})
...
test_that("boolean_xor keeps what is in exactly one", {
  ...
  expect_equal(area(ga_boolean_xor(x, y)), 6)
})
...
test_that("the four operations agree with sf", {
  ...
  expect_equal(
    area(ga_boolean_intersection(x, y)),
    as.numeric(sf::st_area(sf::st_intersection(sa, sb)))
  )
  expect_equal(
    area(ga_boolean_union(x, y)),
    as.numeric(sf::st_area(sf::st_union(sa, sb)))
  )
  expect_equal(
    area(ga_boolean_difference(x, y)),
...
test_that("disjoint inputs give an empty intersection", {
  ...
  expect_equal(area(ga_boolean_intersection(x, y)), 0)
})

test_that("results are always multipolygons", {
  x <- ga(sf::st_sfc(a()))
  y <- ga(sf::st_sfc(b()))
  res <- tosfc(ga_boolean_union(x, y))
...
test_that("boolean ops are length preserving", {
  ...
  expect_length(tosfc(ga_boolean_intersection(x, y)), 3L)
  expect_equal(area(ga_boolean_intersection(x, y)), rep(1, 3))
})
...
test_that("unary_union dissolves the whole array to length one", {
  x <- ga(sf::st_sfc(a(), b()))
  res <- ga_unary_union(x)

  expect_length(tosfc(res), 1L)
  expect_equal(area(res), 7)
})

test_that("unary_union matches folding ga_boolean_union", {
  x <- ga(sf::st_sfc(a(), b()))
  pair <- ga_boolean_union(ga(sf::st_sfc(a())), ga(sf::st_sfc(b())))

  expect_equal(area(ga_unary_union(x)), area(pair))
})
...
test_that("unary_union skips non polygonal rows", {
  x <- ga(sf::st_sfc(a(), sf::st_point(c(0, 0))))
  expect_equal(area(ga_unary_union(x)), 4)
})
