...
test_that("closest_point projects onto a line", {
  ...
  expect_equal(coords(closest_point(g, p)), matrix(c(4, 0), ncol = 2))
})
...
test_that("closest_point returns the intersection when the point is on the line", {
  ...
  expect_equal(coords(closest_point(g, p)), matrix(c(3, 0), ncol = 2))
})
...
test_that("closest_point clamps to the ends of the line", {
  ...
  expect_equal(coords(closest_point(g, p)), matrix(c(0, 10, 0, 0), ncol = 2))
})
...
test_that("closest_point recycles a single point", {
  ...
  expect_length(tosfc(closest_point(g, p)), 3L)
})
...
test_that("closest_point rejects a mismatched point length", {
  ...
  expect_error(closest_point(g, p), "point")
})
...
test_that("closest_point_haversine differs from the planar result", {
  ...
  planar <- coords(closest_point(g, p))
  spherical <- coords(closest_point_haversine(g, p))
...
test_that("line_locate_point returns a fraction of the line", {
  ...
  expect_equal(conv(line_locate_point(g, p)), c(0, 0.25, 1))
})
...
test_that("line_locate_point projects an off-line point", {
  ...
  expect_equal(conv(line_locate_point(g, p)), 0.5)
})

test_that("line_locate_point recycles and validates length", {
  g <- ga(sf::st_sfc(hline(), hline()))

  expect_length(conv(line_locate_point(g, ga(sf::st_sfc(pt(5, 0))))), 2L)
  expect_error(
    line_locate_point(g, ga(sf::st_sfc(pt(0, 0), pt(1, 0), pt(2, 0)))),
    "point"
  )
})
...
test_that("line_locate_point is NA for a geometry with no length", {
  ...
  res <- conv(line_locate_point(g, p))
...
test_that("query functions read a concrete array from a reader", {
  ...
  expect_length(tosfc(interior_point(g)), 100L)
  expect_length(conv(is_convex(g)), 100L)
  expect_length(tosfc(closest_point(g, ga(sf::st_sfc(pt(-79, 35))))), 100L)
})
