...
test_that("crossing lines meet at a point", {
  ...
  res <- tosfc(line_intersection(x, y))
...
test_that("a longer linestring is not a single segment", {
  ...
  expect_equal(line_intersection(x, y)$null_count, 1L)
})
...
test_that("line_intersection recycles and validates", {
  ...
  expect_length(tosfc(line_intersection(x, y1)), 2L)
  expect_error(line_intersection(x, y3), "y")
})

test_that("self_intersections finds the crossing in a bowtie", {
  res <- tosfc(self_intersections(ga(sf::st_sfc(bowtie()))))
