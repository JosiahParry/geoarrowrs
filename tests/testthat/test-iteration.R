...
test_that("coords returns every vertex", {
  res <- tosfc(ga_coords(ga(sf::st_sfc(square()))))

  expect_setequal(as.character(sf::st_geometry_type(res)), "MULTIPOINT")
  expect_equal(n_pts(ga_coords(ga(sf::st_sfc(square())))), 5L)
})

test_that("coords includes interior rings", {
  expect_equal(n_pts(ga_coords(ga(sf::st_sfc(holed())))), 10L)
})

test_that("exterior_coords skips interior rings", {
  expect_equal(n_pts(ga_exterior_coords(ga(sf::st_sfc(holed())))), 5L)
})

test_that("coords is length preserving", {
  g <- ga(sf::st_sfc(square(), square(), square()))
  expect_length(tosfc(ga_coords(g)), 3L)
})

test_that("n_coords agrees with ga_coords", {
  g <- ga(sf::st_sfc(square(), holed()))

  expect_equal(conv(ga_n_coords(g)), c(5L, 10L))
  expect_equal(conv(ga_n_coords(g)), n_pts(ga_coords(g)))
})

test_that("n_coords counts a point as one", {
  expect_equal(conv(ga_n_coords(ga(sf::st_sfc(sf::st_point(c(0, 0)))))), 1L)
})

test_that("lines splits a ring into segments", {
  res <- tosfc(ga_lines(ga(sf::st_sfc(square()))))
...
test_that("lines includes interior rings", {
  expect_equal(lengths(tosfc(ga_lines(ga(sf::st_sfc(holed()))))), 8L)
})
...
test_that("an empty geometry has no coordinates", {
  ...
  expect_equal(conv(ga_n_coords(g)), c(5L, 0L))
  expect_equal(sf::st_is_empty(tosfc(ga_coords(g))), c(FALSE, TRUE))
})
...
test_that("iteration reads a concrete array from a reader", {
  ...
  expect_length(conv(ga_n_coords(g)), 100L)
  expect_length(tosfc(ga_coords(g)), 100L)
  expect_true(all(conv(ga_n_coords(g)) > 0))
})
