...
test_that("the identity transform changes nothing", {
  ...
  res <- ga_affine_transform(g, 1, 0, 0, 0, 1, 0)
...
test_that("it reproduces ga_translate", {
  g <- ga(sf::st_sfc(square()))

  expect_equal(
    sf::st_coordinates(tosfc(ga_affine_transform(g, 1, 0, 5, 0, 1, 3)))[, 1:2],
    sf::st_coordinates(tosfc(ga_translate(g, 5, 3)))[, 1:2]
  )
})
...
test_that("it reproduces scale_xy about the origin", {
  ...
  g <- ga(sf::st_sfc(line))
  scaled <- sf::st_coordinates(tosfc(ga_affine_transform(g, 2, 0, 0, 0, 3, 0)))
...
test_that("coefficients recycle per row", {
  ...
  res <- ga_affine_transform(g, 1, 0, c(0, 10), 0, 1, 0)
